...
const totalBytes = new Counter('total_response_bytes');
...
function handleRequest(name, url) {
    ...
    // Track response times, sizes, and error categories
    ...
    // Check if DNS lookup time exists
    if (res.timings.dns !== undefined) {
        dnsLookup.add(res.timings.dns);
    }

    if (res.timings.tls_handshaking !== undefined) {
        tlsHandshake.add(res.timings.tls_handshaking);
    }

    totalBytes.add(res.body.length);
    requestCount.add(1);
    ...
    if (res.status !== 200) {
        console.error(`[${name}] Error: ${res.status} - ${res.body?.substring(0, 100)}`);
    }
}
